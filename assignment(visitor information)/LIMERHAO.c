...
void binFileFunction(FILE** file, char fileName[30], char mode[3]);
...
void main() {
	...
	/*Sleep(900);
	printf(" __ __  ____ _____ ____  ______   ___   ____       ____  ____   _____   ___   ____   ___ ___   ____  ______  ____  ___   ____       ___ ___   ___   ___    __ __  _        ___ \n");
	Sleep(700);
	printf("|  |  ||    / ___/|    ||      | /   \\ |    \\     |    ||    \\ |     | /   \\ |    \\ |   |   | /    ||      ||    |/   \\ |    \\     |   |   | /   \\ |   \\  |  |  || |      /  _]\n");
	printf("|  |  | |  (   \\_  |  | |      ||     ||  D  )     |  | |  _  ||   __||     ||  D  )| _   _ ||  o  ||      | |  ||     ||  _  |    | _   _ ||     ||    \\ |  |  || |     /  [_ \n");
	Sleep(700);
	printf("|  |  | |  |\\__  | |  | |_|  |_||  O  ||    /      |  | |  |  ||  |_  |  O  ||    / |  \\_/  ||     ||_|  |_| |  ||  O  ||  |  |    |  \\_//  ||  O  ||  D  ||  |  || |___ |    _]\n");
	printf("|  :  | |  |/  \\ | |  |   |  |  |     ||    \\      |  | |  |  ||   _] |     ||    \\ |   |   ||  _  |  |  |   |  ||     ||  |  |    |   |   ||     ||     ||  :  ||     ||   [_ \n");
	Sleep(700);
	printf(" \\   /  |  |\\    | |  |   |  |  |     ||  .  \\     |  | |  |  ||  |   |     ||  .  \\|   |   ||  |  |  |  |   |  ||     ||  |  |    |   |   ||     ||     ||     ||     ||     |\n");
	printf("  \\_/  |____|\\___||____|  |__|   \\___/ |__|\\_|    |____||__|__||__|    \\___/ |__|\\_||___|___||__|__|  |__|  |____|\\___/ |__|__|    |___|___| \\___/ |_____| \\__,_||_____||_____|\n");
...
void addVisitorInformation() {
	...
	binFileFunction(&fptr, "Visitor Information.dat", "ab");
	...
	do {
		...
		do {
			...
			if (validationDate(visitor.day, visitor.month, visitor.year)) {
				printf("\033[0;31m");
				printf("<ERROR>Please enter a valid date<ERROR>\n");
				printf("\033[0m");
			}
			
		} while (validationDate(visitor.day, visitor.month, visitor.year));
		...
		do {
			...
			if (validationTime(visitor.hrs, visitor.min)) {
				printf("\033[0;31m");
				printf("<ERROR>Please enter a valid time visited (eg. 17:20)<ERROR>\n");
				printf("\033[0m");
			}
		} while (validationTime(visitor.hrs, visitor.min));
...
void searchVisitorInformation() {
	Details visitor[100];
	int choice, day, month, year, no = 1, record = 0, count = 0;
	double temperature;
	char venue[20], yesNo;
	FILE* fptr;

	binFileFunction(&fptr, "Visitor Information.dat", "rb");
	...
	do {
		...
		if (toupper(decide) == 'Y') {
			...
			do {
				...
				if (validationDate(visitor[j].day, visitor[j].month, visitor[j].year)) {
					printf("\033[0;31m");
					printf("<ERROR>Please enter a valid date<ERROR>\n");
					printf("\033[0m");
				}

			} while (validationDate(visitor[j].day, visitor[j].month, visitor[j].year));

			do {
				printf("\nEnter New Time Visited (hh:mm) : ");
				scanf("%d:%d", &visitor[j].hrs, &visitor[j].min);

				if (validationTime(visitor[j].hrs, visitor[j].min)) {
					printf("\033[0;31m");
					printf("<ERROR>Please enter a valid time visited (eg. 17:20)<ERROR>\n");
					printf("\033[0m");
				}

			} while (validationTime(visitor[j].hrs, visitor[j].min));
			...
			binFileFunction(&fptr, "Visitor Information.dat", "wb");
